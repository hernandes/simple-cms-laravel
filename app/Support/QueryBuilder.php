...
class QueryBuilder
{
    ...
    {
        $has = !empty($query->getQuery()->wheres);
        $match = isset($data['filter_match']) ? $data['filter_match'] : $this->defaultFilterMatch;

        return $query->when(isset($data['f']), function ($query) use ($data, $has, $match) {
            foreach ($data['f'] as $index => $filter) {
                if ($has && $index === 0 && $match === 'or') {
                    $data['filter_match'] = 'and';
                } else {
                    $data['filter_match'] = $match;
                }
